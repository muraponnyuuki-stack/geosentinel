...
// ニュース5分類：色分けタグ・左側フィルタータブと対応（キーは /classify のレスポンスと一致）
const CATEGORY_META = {
  conflict: { ja: "紛争", color: "#ff9f40" },
  war: { ja: "戦争", color: "#ff4d4d" },
  diplomacy: { ja: "外交", color: "#4fc3ff" },
  policy: { ja: "政策", color: "#b388ff" },
  military_exercise: { ja: "軍事演習", color: "#39ff88" },
};
const activeCategories = new Set(); // 複数選択可。同じタブを再クリックで解除
...
// 現在パネルに表示中の記事（分類カテゴリー付き）。フィルタータブの再クリック時に
// 再取得・再分類せず、この配列を元に即座に絞り込み再描画する。
let currentArticles = [];
let currentLabel = "";
let isClassifying = false; // 分類APIの応答待ち中（フィルター選択時のみ意味を持つ）

// 現在選択中の国/地域（期間タブ・ページ切り替え時に /news を再構築するための情報）
let currentQuery = null; // { country, region, classify, summaryRegion }
let currentRange = "1w";
let currentPage = 1;
...
// ===== 期間フィルター・ページネーションの表示制御 =====
function resetRangeUI() {
  currentRange = "1w";
  currentPage = 1;
  const rangeTabs = document.getElementById("range-tabs");
  if (rangeTabs) {
    rangeTabs.hidden = false;
    rangeTabs.querySelectorAll(".range-tab").forEach((b) => {
...
function hideRangeUI() {
  const rangeTabs = document.getElementById("range-tabs");
  const pagination = document.getElementById("panel-pagination");
  if (rangeTabs) rangeTabs.hidden = true;
...
async function showCountry(iso2, name, continent) {
  ...
  currentQuery = { country: iso2, region: null, classify: true, summaryRegion: name };
  resetRangeUI();
  await runFeed(label);
}
...
async function showRegion(region) {
  ...
  panelBody.innerHTML =
    ...
    "</div>";
  currentQuery = {
    country: isJapan ? "JP" : null,
    region: isJapan ? null : region,
    classify: false,
    summaryRegion: isJapan ? "日本" : region,
  };
  resetRangeUI();
  await runFeed(ja);
}

// ニュース取得（期間・ページ指定）→ 5分類（色分け） → 要約 → 右パネルに記事一覧、下部に地域要約。
// 期間タブ・ページネーションのクリック時もこの関数を呼び直す（currentQuery は保持したまま）。
async function runFeed(label) {
  if (!currentQuery) return;
  const { country, region, classify, summaryRegion } = currentQuery;
  currentLabel = label;
  ...
  try {
    ...
    if (!classify) showSummaryStatus(label, "ニュース取得に失敗しました");
    renderPagination(1, 1);
    return;
  }

  const articles = data.articles || [];
  renderPagination(data.total_pages || 1, data.page || currentPage);

  if (!articles.length) {
    currentArticles = [];
    isClassifying = false;
    renderPanel(label, [], "該当ニュースなし。");
    if (!classify) showSummaryStatus(label, "該当ニュースなし");
    return;
  }

  // まず記事一覧を即時表示（分類前。分類フィルター選択中は読み込み中と表示する）
  let used = articles;
  currentArticles = used;
  isClassifying = activeCategories.size > 0;
  applyFilterAndRender();

  // 5分類（紛争/戦争/外交/政策/軍事演習）で色分け・フィルター用のカテゴリーを付与
  const cats = await Promise.all(used.map(classifyArticle));
  used.forEach((a, i) => { a.category = cats[i]; });
  currentArticles = used;
  isClassifying = false;
  applyFilterAndRender();

  // /summarize で地域要約を生成（地域タブのみ、国クリック時はスキップ）。
  // フィルター選択中はその絞り込み結果を要約対象にする。
  if (!classify) {
    const filtered = activeCategories.size
      ? used.filter((a) => activeCategories.has(a.category))
      : used;
    const forSummary = filtered.length ? filtered : used;
    showSummaryStatus(label, "要約を生成中…");
    try {
      const sum = await apiPost("/summarize", {
        region: summaryRegion,
        articles: forSummary.map((a) => ({
          title: a.title || "",
...
// 現在の記事一覧（currentArticles）に、選択中のカテゴリーフィルターを適用して再描画。
// フィルタータブのクリック時はここだけを呼び、API を叩き直さない。
function applyFilterAndRender() {
  ...
  if (activeCategories.size > 0) {
    if (isClassifying) {
      // 分類APIの応答待ち中はいったん「該当なし」と誤表示しないよう、読み込み中を出す
      loading = true;
      list = [];
    } else {
      list = currentArticles.filter((a) => a.category && activeCategories.has(a.category));
      if (!list.length) note = "選択中の分類に該当するニュースはありません。";
    }
  }
  renderPanel(currentLabel, list, note, loading);
}

// 右パネル：記事カード（リンク付き・分類色タグ付き）
function renderPanel(label, articles, note, loading) {
  if (loading) {
    panelBody.innerHTML =
      '<div class="panel-placeholder">' + esc(label) + "</div>" +
      '<p class="panel-loading">分類を読み込み中です…</p>';
    return;
  }
  ...
  for (const a of articles) {
    ...
    const catTag = cat ? '<span class="ni-cat"' + catStyle + '>' + esc(cat.ja) + "</span>" : "";
    html +=
      '<div class="news-item"' + catStyle + '>' + catTag +
      '<a class="ni-link" href="' + esc(url) +
...
// ===== 期間フィルタータブ（3日/1週間/1か月/3か月/6か月/1年・単一選択） =====
function bindRangeTabs() {
  document.querySelectorAll(".range-tab").forEach((btn) => {
    btn.addEventListener("click", () => {
      if (btn.classList.contains("active")) return;
      document.querySelectorAll(".range-tab.active").forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
      currentRange = btn.dataset.range;
      currentPage = 1;
      runFeed(currentLabel);
    });
  });
}

// ===== ページネーション（期間内の記事が21件以上のとき表示） =====
function renderPagination(totalPages, page) {
  ...
  html += '<button class="page-btn" data-page="' + Math.min(totalPages, page + 1) + '"' +
...
// ===== 左側：ニュース分類フィルタータブ（複数選択可・再クリックで解除） =====
function bindCategoryTabs() {
  document.querySelectorAll(".category-tab").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      applyFilterAndRender(); // 再取得はせず、現在の記事を絞り込み直す
    });
  });
}
